...
const config = {
  name: "CNN",
  desc: "Berita Dari CNN",
  path: "/berita/cnn"
};

const apiUrl = "https://api.siputzx.my.id/api/berita/cnn";
...
async function fetchCnnNews() {
  try {
    ...
    console.error("Error fetching CNN news:", error.message);
    return {
      status: false,
...
module.exports = function (app) {
  app.get(config.path, async (req, res) => {
    try {
      const results = await fetchCnnNews();
      if (results.status) {
        res.status(200).json({
          status: true,
          creator: "Xyz-King's",
      ...
      console.error("Server error at /berita/cnn:", error.message);
      res.status(500).json({
        status: false,
        creator: "Xyz-King's",
